...
RATIO_LABEL = r"SM/EFT"
MASS_RATIO_YLIM = (0.85, 1.15)
Y_RATIO_YLIM = (0.95, 1.05)
...
class SampleData:
    ...
    def combined_sigma_pb(self) -> float:
        ...
        For two equal-statistics runs, this is simply the arithmetic mean.
        """
...
def load_sample(
    ...
    if len(files) != len(cross_sections_pb):
        raise ValueError(
            f"Sample {name}: number of files and cross sections differ."
        )
    ...
    for path, sigma_pb in zip(files, cross_sections_pb):
        ...
        if len(masses) == 0:
            raise RuntimeError(
                f"No W+W- events were reconstructed from file:\n{path}"
            )
...
def make_histogram(
    ...
    If differential=True:
        returns dσ/dx with uncertainties.

    If differential=False:
        returns bin-integrated σ_bin with uncertainties.
    """
...
def save_histogram_csv(
    ...
    header = (
        ...
        "SM,SM_MCerr,EFT,EFT_MCerr,SM_over_EFT"
    )
...
def setup_ratio_axes(figsize=(8.0, 9.0)):
    ...
    plt.subplots_adjust(left=0.15, right=0.95, bottom=0.12, top=0.93)
...
def plot_mass_distribution(sm: SampleData, eft: SampleData) -> None:
    ...
    eft_hist, eft_err = make_histogram(
        eft_values[eft_mask],
        eft_weights[eft_mask],
        bins,
        differential=True,
    )
...
def plot_rapidity_distribution(sm: SampleData, eft: SampleData) -> None:
    ...
    rax.set_ylabel(RATIO_LABEL, fontsize=20)
    rax.set_xlabel(r"$Y_{W^+W^-}$", fontsize=24)
    rax.set_xlim(Y_MIN, Y_MAX)
    rax.set_ylim(*Y_RATIO_YLIM)
    style_ratio_axis(rax)
...
def main() -> None:
    ...
    print("=" * 80)
    print(r"Observable 1: dσ/dM_{W+W-}, up to 1000 GeV")
    print(r"Observable 2: dσ/dY_{W+W-}")
    print(r"Ratio panel : SM/EFT")
    ...
    print(
        f"Inclusive SM/EFT ratio       = "
        f"{sm_sample.combined_sigma_pb / eft_sample.combined_sigma_pb:.9f}"
    )
    print("-" * 80)
