...
#    yp = w * w / s_cms / ye
    ...
    if (yp <= 0.01 or yp >= 0.1 or ye <= 0.0 or ye >= 1.0):          # Hamzeh take care of tagged elastic    if (yp <= 0.01 or yp >= 0.20):
    ...
    flux_prod = cs_ww_w(w) * flux_y_dipole(yp, pmass, qmax2p) \
...
#    yp = w * w / s_cms / ye
    ...
    # according to Eq.(A.1)
    flux_prod = cs_ww_w(w) * flux_y_inel(yp, minM2, qmax2p, mNmax) \
                * w * flux_y_pl(ye, emass, qmax2e)
    if pout:
